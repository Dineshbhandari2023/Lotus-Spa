...
import Header from "@/components/Header"; // Importing your professional header
import {
  Clock,
  Receipt,
  Sparkles,
...
// Self-contained therapy database
const therapiesData = {
  ayurvedic: {
    EN: {
...
export default function TherapyPage() {
  ...
  // Sync state with Home Hub choices via LocalStorage
  useEffect(() => {
    setIsMounted(true);
    const storedTheme = localStorage.getItem("lotus_theme") as
      | "light"
      | "dark"
      | null;
  ...
  const content = therapy[lang];

  // Serenity Hydration Guard
  if (!isMounted) {
    return (
      <div className="min-h-screen bg-[#121510] flex flex-col items-center justify-center text-white/80 font-sans text-xs uppercase tracking-[0.2em]">
        <div className="flex gap-2 items-center">
          <span
            className="w-1.5 h-1.5 bg-[#7C8461] animate-bounce"
  ...
  return (
    ...
    >
      {/* 
        Integrated your Global Professional Header 
        This includes the Therapies Dropdown, Language Toggle, and Theme Toggle
      */}
      <Header
        currentLang={lang}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            >
              <div className="space-y-1">
                <span className="text-[9px] font-bold uppercase tracking-widest text-[#7C8461] block">
                  {lang === "EN" ? "Therapy Price" : "शुल्क लागत"}
                </span>
              ...
              <div className="space-y-1">
                <span className="text-[9px] font-bold uppercase tracking-widest text-[#7C8461] block">
                  {lang === "EN" ? "Treatment Duration" : "समय अवधि"}
                </span>
                <span className="text-xl font-black flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-20 pt-16 border-t border-dashed border-[#E5DED4] dark:border-[#272F22]">
          <div className="space-y-6">
            <h3 className="text-lg font-black uppercase tracking-wider text-[#7C8461]">
              {lang === "EN" ? "Therapy Benefits" : "फाइदाहरू"}
            </h3>
          ...
          <div className="space-y-6">
            <h3 className="text-lg font-black uppercase tracking-wider text-[#7C8461]">
              {lang === "EN" ? "Appointment Preparation" : "पूर्व तयारी नियम"}
            </h3>
            <div className="space-y-4">
        ...
        <Footer currentLang={"EN"} theme={"light"} />
      </main>
    </div>
  );
}
