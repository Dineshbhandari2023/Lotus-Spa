...
export default function Header({ currentLang, setLang, theme, setTheme }: HeaderProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`hidden sm:flex text-xs py-1.5 px-4 justify-between items-center transition-colors border-b ${
        theme === 'dark' ? 'bg-[#1A1F16]/60 text-[#A2A994] border-[#272F22]' : 'bg-[#F8F6F2] text-[#555] border-[#E5DED4]'
      }`}>
        ...
        <div className="flex items-center gap-3 font-sans tracking-[0.1em] uppercase text-[10px]">
          <span>{t.phone}: <strong className="text-[#7C8461]">+977-9801205933</strong></span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <motion.div
            ...
            className={`p-2 rounded-none border ${
              theme === 'dark' ? 'bg-[#1A1F16] text-[#7C8461] border-[#272F22]' : 'bg-[#F8F6F2] text-[#7C8461] border-[#E5DED4]'
            }`}
          >
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-6 mr-4 text-[11px] uppercase tracking-[0.2em] font-sans font-semibold">
            <a href="#services-section" className="hover:text-[#7C8461] transition-colors">{t.services}</a>
            <a href="#branches-section" className="hover:text-[#7C8461] transition-colors">{t.branchesTitle}</a>
            <a href="#booking-section" className="hover:text-[#7C8461] transition-colors">{t.booking}</a>
            <a href="#why-us-section" className="hover:text-[#7C8461] transition-colors">{t.whyUs}</a>
          </nav>
